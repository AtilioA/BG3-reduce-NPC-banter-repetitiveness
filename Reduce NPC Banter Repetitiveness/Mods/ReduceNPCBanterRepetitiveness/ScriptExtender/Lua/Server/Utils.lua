...
function Utils.DebugPrint(level, ...)
  if JsonConfig and JsonConfig.DEBUG and JsonConfig.DEBUG.level >= level then
    if (JsonConfig.DEBUG.level == 0) then
      print("[Reduce NPC Banter Repetitiveness]: " .. ...)
    else
      print("[Reduce NPC Banter Repetitiveness][D" .. level .. "]: " .. ...)
    end
  end
end
...
function Utils.DumpCharacterEntity(character)
  if JsonConfig.DEBUG.level >= 2 then
    ...
    Ext.IO.SaveFile('character-entity-ReduceNPCBanterRepetitiveness.json', Ext.DumpExport(charEntity:GetAllComponents()))
  end
end
